...
savepath_latency_fig = r'E:\Manuscript_analysis_files\LC_seizure_python_scripts\Figure04\output\Fig4D.png'
...
data_sigificant = data[data['significant_parametric'] == 1]
...
histrogram = np.histogram(np.abs(data_sigificant['Latencies']), bins=20, range=(0,200))
x = np.arange(0,200, 10) + 5
...
ax.bar(x, histrogram[0], width = 9, color='gray', label = 'Broad')
...
ax.set_xlabel('Peak interaction (ms)', fontsize=14, fontweight='bold')
ax.set_ylabel('#pairs', fontsize=14, fontweight='bold')
#ax.set_ylim((0, 0.6))
#ax.legend(frameon=False, loc='upper right')
plt.tight_layout()       
plt.savefig(savepath_latency_fig, dpi=600)
