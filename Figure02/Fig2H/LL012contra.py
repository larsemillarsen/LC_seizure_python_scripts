...
for i in range(len(firing_criteria)):
    if np.min(spike_count_sz1[35:59,i]) > min_firing and np.min(spike_count_sz2[0:59,i]) > min_firing and np.min(spike_count_sz3[0:59,i]) > min_firing:
        firing_criteria[i] = 1
...
#    spike_count = np.mean(spike_count, axis=1)
    ...
    basemean_spike_count_sz1 = np.mean(spike_count_sz1[35:59])
    basemean_std_sz1 = np.std(spike_count_sz1[35:59])
